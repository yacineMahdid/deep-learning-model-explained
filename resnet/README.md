...
The gist of this function is as its name implies, we are going to create the layered blocks and return them:
```python
        layers = []
...
Before that happens we do some setup for our downsampling functions, one of which is a dilation which can be understood as follows:

![Dilation in action](images/dilation.gif)

Cyan is output and blue is input, the algorithm is weird but you can check out [more information here](https://github.com/vdumoulin/conv_arithmetic/blob/master/README.md).
...
```
Nothing too critical here to understand, the main thing to know is that we setting up for downsampling we have do in the blocks we already discussed.

All of this brings us to the meat of the functions which is this part:
```python
        layers = []
...
1. We first appends one block, which has the downsampling parameter attached to.
2. we modify the input channel to be the numbers of channel we had in the first block * expansion parameters. Either 1 for BasicBlock (aka won't do anything) and 4 for the BottleneckBlock.
3. Then we iteratively create blocks that have the same format. 

The blocks parameters dictacting the number of block created are these in the paper:

![Blocks parameters](images/blocks_parameters.png)

and we can see it being used way back up in the call stack `return _resnet(BasicBlock, [2, 2, 2, 2], weights, progress, **kwargs)` (i.e. [2, 2, 2, 2] for resnet18)

Let's now look at the initialization
...
Again, lots of stuff going on and most of it has to do with Pytorch internals and various (very useful) optimization.
The only interesting part is this one:
...
```python
        ...
        self.fc = nn.Linear(512 * block.expansion, num_classes)
```
These are simply the implementation details we've seen of the network structure in the `_forward` function.

## Conclusion
Voila! Altough a bit dauting in term of size, the important bits of code for the architecture is actually well contained in the BasicBlock and Bottlneck block class.
